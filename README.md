...
Source: [UCI Heart Failure Prediction Dataset](https://www.kaggle.com/datasets/fedesoriano/heart-failure-prediction)
...
- Feature scaling with StandardScaler for distance- and margin-based models (SVM, KNN)
- Train-test split (80 percent train, 20 percent test, with a fixed random state for reproducibility)

### 4. Model Training and Evaluation
Six classification approaches were trained and evaluated on the same train-test split:
...
- K-Nearest Neighbors
- Gradient Boosting
- Voting Ensemble (hard voting across Logistic Regression, SVM, Decision Tree, and KNN)
...
Note: some metrics were not computed for every model in the current version of the notebook. Standardizing the evaluation across all models (accuracy, precision, recall, F1, and confusion matrix) is listed as a planned improvement below.

The Support Vector Machine achieved the highest standalone accuracy, while the Voting Ensemble delivers consistent, competitive performance by combining the predictions of four different models.

## Planned Improvements

- Standardize evaluation metrics (accuracy, precision, recall, F1, confusion matrix, ROC-AUC) across all models
...
- Explore soft voting and stacking ensembles in addition to hard voting

## Desktop Application

HeartGuard includes a desktop interface that makes the trained model accessible to non-technical users. The application collects the same clinical inputs used during training (age, sex, chest pain type, resting blood pressure, cholesterol, fasting blood sugar, resting ECG, maximum heart rate, exercise angina, oldpeak, and ST slope) through a patient information form and returns a real-time prediction alongside the model's probability score.

The trained model and its fitted scaler are loaded by the application at startup and used to generate predictions on demand, so no internet connection or external service is required at inference time.
...
- Desktop GUI framework for the HeartGuard application
...
1. Clone the repository and install the required packages:
   ...
   ```
2. To review or retrain the model, ensure `heart.csv` is in the `data` directory and open `PROJECT_ML.ipynb` in Jupyter Notebook or JupyterLab.
3. To run the desktop application, launch `heartguard_app.py` from the `app` directory. The application loads the trained model and scaler from the `models` directory automatically.
